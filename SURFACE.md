...
| `certificate-repair-check` | check a repair: the parent defect, the candidate proof, the inherited contract | 3 | 4 | 0 | 2 |
| `certificate-repair-pack` | package a refutation and a candidate certificate as an unchecked repair | 1 | 1 | 0 | 1 |
...
| `machine-create` | build a machine from a specification and write its bytes | 2 | 2 | 0 | 1 |
| `machine-discover` | report the bit properties that hold at every reached state | 1 | 0 | 0 | 0 |
| `machine-evidence` | produce a certificate or a refutation for a machine and write it | 3 | 3 | 0 | 2 |
...
| `model-project` | write a machine's full transition table as a canonical projection, unchecked | 4 | 1 | 0 | 1 |
| `policy` | compile a boolean WPL file and sign its decision | 25 | 0 | 1 | 0 |
| `projection-check` | check a projection row by row against a verified certificate of its model | 2 | 1 | 0 | 2 |
| `projection-checker` | identify the independent projection checker | 1 | 3 | 0 | 1 |
| `projection-materialize` | write a projection next to the fixed table runtime; verify nothing | 3 | 1 | 0 | 1 |
...
| `repair-search` | search a bounded neighborhood for a certified model repair | 2 | 2 | 0 | 1 |
